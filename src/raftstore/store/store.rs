...
impl<T: Transport, C: PdClient> Store<T, C> {
    ...
    fn on_ready_split_region(&mut self,
                             ...
                             left: metapb::Region,
                             right: metapb::Region) {
        self.region_peers.get_mut(&region_id).unwrap().mut_store().region = right.clone();
        ...
        match Peer::create(self, &left) {
            ...
            Ok(mut new_peer) => {
                ...
                if is_leader {
                    ...
                    let right = &self.region_peers[&region_id];
                    self.report_split_pd(right, &new_peer);
                }
